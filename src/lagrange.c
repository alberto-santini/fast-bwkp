...
struct lagrange_solution solve_lagrange(const struct instance* inst, float multiplier, double* used_time_ms, enum sorting_method mtd) {
    ...
    if(mtd == HEAPSORT) {
        ...
        in_place_quickersort_desc(&lagr_p_black, &w_black, inst->n_black);
        in_place_quickersort_desc(&lagr_p_white, &w_white, inst->n_white);
    }
...
struct lagrange_solution solve_for_all_pw(struct instance* inst, double* used_time_ms, enum sorting_method mtd) {
    ...
    *used_time_ms = 0;
    ...
    struct lagrange_solution sol = solve_lagrange(inst, 0.0, used_time_ms, mtd);
    ...
    for(ptrdiff_t i = 0; i < n_multipliers && sol.capacity_violation >= 0; i++) {
        sol = solve_lagrange(inst, multipliers[i], used_time_ms, mtd);
